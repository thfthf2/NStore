...
namespace NStore.Web.StoreAdmin.Controllers
{
    ...
    public partial class ProductController : BaseStoreAdminController
    {
        ...
        {
            if (!SecureHelper.IsSafeSqlString(productName)) productName = "";
            ...
            itemList.Add(new SelectListItem() { Text = "全部分类", Value = "0" });
            foreach (StoreClassInfo storeClassInfo in AdminStores.GetStoreClassList(WorkContext.StoreId).FindAll(x => x.HasChild == 0))
            {
                itemList.Add(new SelectListItem() { Text = storeClassInfo.Name, Value = storeClassInfo.StoreCid.ToString() });
            }
            ViewData["storeClassList"] = itemList;
        ...
        /// 添加商品关键词
        /// </summary>
        public ActionResult AddProductKeyword(string keyword, int pid = -1)
        {
            PartProductInfo partProductInfo = AdminProducts.AdminGetPartProductById(pid);
            if (partProductInfo == null)
                return PromptView("商品不存在");
