...
namespace NStore.Data
{
    ...
    public partial class OAuths
    {
        ...
        {
            return NStore.Core.BMAData.RDBS.CreateOAuthUser(oauthInfo);
        }
        ...
        public static int GetUidByOpenIdAndServer(string openId, string server)
        {
            ...
            if (_usernosql != null)
            {
                ...
                if (uid < 1)
                {
                    uid = NStore.Core.BMAData.RDBS.GetUidByOpenIdAndServer(openId, server);
                    if (uid > 0)
        ...
        public static OAuthInfo GetOAuthUserByUid(int uid)
        {
            ...
            IDataReader reader = NStore.Core.BMAData.RDBS.GetOAuthUserByUid(uid);
            if (reader.Read())
            {
        ...
        public static List<OAuthInfo> GetOAuthUserList(string uidList)
        {
            ...
            IDataReader reader = NStore.Core.BMAData.RDBS.GetOAuthUserList(uidList);
            while (reader.Read())
            {
