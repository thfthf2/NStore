...
namespace NStore.Core
{
    ...
    public static partial class BMACache
    {
        ...
        static BMACache()
        {
            try
            {
                string[] fileNameList = Directory.GetFiles(System.Web.HttpRuntime.BinDirectory, "NStore.CacheStrategy.*.dll", SearchOption.TopDirectoryOnly);
                _icachestrategy = (ICacheStrategy)Activator.CreateInstance(Type.GetType(string.Format("NStore.CacheStrategy.{0}.CacheStrategy, NStore.CacheStrategy.{0}", fileNameList[0].Substring(fileNameList[0].IndexOf("CacheStrategy.") + 14).Replace(".dll", "")),
                                                                                       false,
                                                                                       true));
            }
            catch
            {
                throw new BMAException("创建'缓存策略对象'失败，可能存在的原因：未将'缓存策略程序集'添加到bin目录中；'缓存策略程序集'文件名不符合'NStore.CacheStrategy.{策略名称}.dll'格式");
            }
