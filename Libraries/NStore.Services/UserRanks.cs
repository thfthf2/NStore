...
namespace NStore.Services
{
    ...
    public partial class UserRanks
    {
        ...
        {
            List<UserRankInfo> userRankList = NStore.Core.BMACache.Get(CacheKeys.MALL_USERRANK_LIST) as List<UserRankInfo>;
            if (userRankList == null)
            {
                userRankList = NStore.Data.UserRanks.GetUserRankList();
                NStore.Core.BMACache.Insert(CacheKeys.MALL_USERRANK_LIST, userRankList);
            }
            return userRankList;
        }
