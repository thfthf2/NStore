...
namespace NStore.Services
{
    ...
    public partial class Helps
    {
        ...
        {
            List<HelpInfo> helpTree = NStore.Core.BMACache.Get(CacheKeys.MALL_HELP_LIST) as List<HelpInfo>;

            if (helpTree == null)
            {
                helpTree = new List<HelpInfo>();
                List<HelpInfo> helpList = NStore.Data.Helps.GetHelpList();

                CreateHelpTree(helpList, helpTree, 0);
                NStore.Core.BMACache.Insert(CacheKeys.MALL_HELP_LIST, helpTree);
            }
            return helpTree;
        }
