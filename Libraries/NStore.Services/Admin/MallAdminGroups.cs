...
namespace NStore.Services
{
    ...
    public partial class MallAdminGroups
    {
        ...
        {
            HashSet<string> actionHashSet = NStore.Core.BMACache.Get(CacheKeys.MALL_MALLADMINGROUP_ACTIONHASHSET + mallAGid) as HashSet<string>;
            if (actionHashSet == null)
            {
                ...
                if (mallAdminGroupInfo != null)
                {
                    ...
                    NStore.Core.BMACache.Insert(CacheKeys.MALL_MALLADMINGROUP_ACTIONHASHSET + mallAGid, actionHashSet);
                }
            }
        ...
        {
            MallAdminGroupInfo[] mallAdminGroupList = NStore.Core.BMACache.Get(CacheKeys.MALL_MALLADMINGROUP_LIST) as MallAdminGroupInfo[];
            if (mallAdminGroupList == null)
            {
                mallAdminGroupList = NStore.Data.MallAdminGroups.GetMallAdminGroupList();
                NStore.Core.BMACache.Insert(CacheKeys.MALL_MALLADMINGROUP_LIST, mallAdminGroupList);
            }
        ...
        public static void CreateMallAdminGroup(MallAdminGroupInfo mallAdminGroupInfo)
        {
            ...
            int mallAGid = NStore.Data.MallAdminGroups.CreateMallAdminGroup(mallAdminGroupInfo);
            if (mallAGid > 0)
            {
        ...
        {
            mallAdminGroupInfo.ActionList = mallAdminGroupInfo.ActionList.ToLower();
            NStore.Data.MallAdminGroups.UpdateMallAdminGroup(mallAdminGroupInfo);
            NStore.Core.BMACache.Remove(CacheKeys.MALL_MALLADMINGROUP_ACTIONHASHSET + mallAdminGroupInfo.MallAGid);
            NStore.Core.BMACache.Remove(CacheKeys.MALL_MALLADMINGROUP_LIST);
            WriteMallAdminNavMenuCache(mallAdminGroupInfo);
        }
