...
namespace NStore.Services
{
    ...
    public partial class AdminShipCompanies : ShipCompanies
    {
        ...
        {
            NStore.Data.ShipCompanies.DeleteShipCompanyById(shipCoId);
            NStore.Core.BMACache.Remove(CacheKeys.MALL_SHIPCOMPANY_INFO + shipCoId);
        }
        ...
        public static int GetShipCoIdByName(string shipCoName)
        {
            if (string.IsNullOrWhiteSpace(shipCoName))
                return 0;
            return NStore.Data.ShipCompanies.GetShipCoIdByName(shipCoName);
        }
    }
}
