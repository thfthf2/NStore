...
namespace NStore.Services
{
    ...
    public partial class Navs
    {
        ...
        {
            List<NavInfo> navTree = NStore.Core.BMACache.Get(CacheKeys.MALL_NAV_LIST) as List<NavInfo>;
        ...
        {
            List<NavInfo> navList = NStore.Core.BMACache.Get(CacheKeys.MALL_NAV_MAINLIST) as List<NavInfo>;

            if (navList == null)
            {
                navList = new List<NavInfo>();
                foreach (NavInfo navInfo in NStore.Data.Navs.GetNavList())
                {
                    if (navInfo.Pid == 0)
                        navList.Add(navInfo);
                }
                NStore.Core.BMACache.Insert(CacheKeys.MALL_NAV_MAINLIST, navList);
            }
            return navList;
        }
