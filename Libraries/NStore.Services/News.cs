...
namespace NStore.Services
{
    ...
    public partial class News
    {
        ...
        {
            List<NewsTypeInfo> newsTypeList = NStore.Core.BMACache.Get(CacheKeys.MALL_NEWSTYPE_LIST) as List<NewsTypeInfo>;
            if (newsTypeList == null)
            {
                newsTypeList = NStore.Data.News.GetNewsTypeList();
                NStore.Core.BMACache.Insert(CacheKeys.MALL_NEWSTYPE_LIST, newsTypeList);
            }
            return newsTypeList;
        }
        ...
        {
            if (newsId > 0)
                return NStore.Data.News.GetNewsById(newsId);
            return null;
        }
        ...
        {
            DataTable newsList = NStore.Core.BMACache.Get(CacheKeys.MALL_NEWS_HOMELIST + newsTypeId) as DataTable;
            if (newsList == null)
            {
                newsList = NStore.Data.News.GetHomeNewsList(newsTypeId, count);
                NStore.Core.BMACache.Insert(CacheKeys.MALL_NEWS_HOMELIST + newsTypeId, newsList);
            }
            return newsList;
        }
        ...
        {
            return NStore.Data.News.GetNewsListCondition(newsTypeId, title);
        }
        ...
        {
            return NStore.Data.News.GetNewsList(pageSize, pageNumber, condition);
        }
        ...
        {
            return NStore.Data.News.GetNewsCount(condition);
        }
    }
}
