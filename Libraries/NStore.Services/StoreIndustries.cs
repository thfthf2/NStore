...
namespace NStore.Services
{
    ...
    public partial class StoreIndustries
    {
        ...
        {
            List<StoreIndustryInfo> storeIndustryList = NStore.Core.BMACache.Get(CacheKeys.MALL_STORE_INDUSTRYLIST) as List<StoreIndustryInfo>;
            if (storeIndustryList == null)
            {
                storeIndustryList = NStore.Data.StoreIndustries.GetStoreIndustryList();
                NStore.Core.BMACache.Insert(CacheKeys.MALL_STORE_INDUSTRYLIST, storeIndustryList);
            }
